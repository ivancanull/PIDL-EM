...
class CFSParameters:
    ...
    def _compute_scaled_distance(npml):
        ...
        rho = (torch.arange(0, 2 * npml) + 0.5) / (2 * npml) # 0.5 / (2 * npml) ~ (2 * npml + 0.5) / (2 * npml)
        rho_e = torch.vstack([rho.flip(dims=(-1,))[1::2], rho[0::2]]) # 
        rho_m = torch.vstack([rho.flip(dims=(-1,))[0::2], rho[1::2]])
        return rho_e, rho_m
    ...
    def get_update_coefficients(self, nx, dx, dt, epsr):
        ...
        """
        sigma_opt = (self.order + 1) / ((150. * np.pi * torch.sqrt(epsr) * dx))
        ...
        be = torch.exp(-(alpha_e + sigma_e / kappa_e) * (dt / epsilon0))
        ae = (be - 1.) * (sigma_e / (sigma_e + alpha_e * kappa_e))
        bm = torch.exp(-(alpha_m + sigma_m / kappa_m) * (dt / mu0))
        am = (bm - 1.) * (sigma_m / (sigma_m + alpha_m * kappa_m))

        return self._extend_dims(torch.vstack([be, ae, kappa_e, bm, am, kappa_m]), nx)

    def _extend_dims(self, paras, ncells):
        """extend dimensions"""
        ce = torch.zeros((3, ncells + 1,))  # be, ae, ke
        cm = torch.zeros((3, ncells,))  # bm, am, km
        npml = self.npml
