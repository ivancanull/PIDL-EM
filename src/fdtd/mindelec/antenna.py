...
class Antenna():
    ...
    def get_outputs_at_each_step(self, hidden_states):
        ...
        voltages = torch.stack(voltages, dim=-1)
        ...
        currents = torch.stack(currents, dim=-1)

        return torch.stack([voltages, currents], dim=-1)

    def generate_surface_img(self, 
                             origin: Sequence[int]) -> np.ndarray:
        """
        Gereate surface image with PEC and uniform block.
        
        Support:
        - PEC plate on one surface and one direction
        - PEC = 1    uniform block = 0
        - remove npml and air buffers
        - only keep the last layer at -origin[2]
        ...
        img = np.zeros(self.cell_numbers)
        for obj in self.grid_helper.objects_on_faces:
            (i_s, i_e), (j_s, j_e), (k_s, k_e) = obj.indices
